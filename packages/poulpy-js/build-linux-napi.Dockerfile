# Linux-only glibc napi build for `poulpy-js` (e.g. `poulpy.linux-x64-gnu.node` for Vercel / linux/amd64).
...
#   docker build --platform=linux/amd64 -f packages/poulpy-js/build-linux-napi.Dockerfile -t poulpy-linux:local .
# Baked build + copy napi/ into this package (runs docker build, then copies napi/ out; see scripts/build-linux-napi.sh):
#   pnpm --filter poulpy-js run build:poulpy:linux
#   (from packages/poulpy-js: pnpm run build:poulpy:linux)
#
# Mount workflow (from monorepo root; rewrites the tree under /poulpy-js):
#   docker run --rm -v "$PWD":/poulpy-js -w /poulpy-js poulpy-linux:local \
#     bash -c 'pnpm install --no-frozen-lockfile && pnpm run build:poulpy'
#
# The image is built on Debian bookworm, Node 20, Rust nightly (see /poulpy-js/rust-toolchain.toml for wasm), wasm-pack, pnpm@9.12.0.
...
RUN pnpm install --no-frozen-lockfile && pnpm run build:poulpy
